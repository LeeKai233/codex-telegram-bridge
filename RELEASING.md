...
2. Build assets from the CI-passed commit and prepare checksums outside the checkout:
   ...
   cp install.sh dist/codex_telegram_bridge-0.3.1-py3-none-any.whl \
      dist/codex_telegram_bridge-0.3.1.tar.gz "$release_dir/"
...
3. Create a draft targeting the exact commit. Creating the release creates the tag; do not push a
   ...
   gh release create v0.3.1 --repo LeeKai233/codex-telegram-bridge \
      --draft --target "$release_sha" --title "v0.3.1" \
      "$release_dir/install.sh" "$release_dir/SHA256SUMS" \
      "$release_dir/codex_telegram_bridge-0.3.1-py3-none-any.whl" \
      "$release_dir/codex_telegram_bridge-0.3.1.tar.gz"
   ```
...
release_dir="$(mktemp -d)"
cd "$release_dir"
base="https://github.com/LeeKai233/codex-telegram-bridge/releases/download/v0.3.1"
for asset in install.sh SHA256SUMS \
  codex_telegram_bridge-0.3.1-py3-none-any.whl \
  codex_telegram_bridge-0.3.1.tar.gz; do
...
UV_TOOL_DIR="$release_dir/tools" UV_TOOL_BIN_DIR="$release_dir/bin" \
  ...
  git+https://github.com/LeeKai233/codex-telegram-bridge@v0.3.1
